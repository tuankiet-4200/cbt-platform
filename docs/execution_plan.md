...
#### Backend Deliverables
- [x] **Section-scoped lifecycle architecture:** One `ExamAttempt` aggregates sequential MATH, READING, and SCIENCE `ExamSession` records. Each section has its own server-authoritative timer (defaults: 60/30/60 minutes) and transition state. See [section-session-architecture.md](./section-session-architecture.md).
...
- [x] **Answer Sync API (Redis Buffer):**
...
- [x] **Session State Recovery:** `GET /api/v1/sessions/:id/state` → Lấy từ Redis (nếu còn) hoặc PostgreSQL (fallback).
...
- [x] **Zustand `examStore`:**
  ...
  interface ExamStore {
    sessionId: string;
    answers: Record<string, unknown>; // questionId → answerJson
...
- [x] **Sync Service:** Debounce 3s, batch `POST /api/v1/sessions/:id/sync`. Offline queue với `navigator.onLine` event.
- [x] **Timer:** Server-authoritative section countdown — đổi màu đỏ khi < 5 phút. Tính từ section `endTime - Date.now()` (không decrement thuần túy).
- [x] **Section transition:** Confirmation before each section plus completion summary and explicit continuation to the next section, matching the approved TSA reference.
...
- [x] **Grading Worker (BullMQ queue: `grading-queue`, job: `grade-attempt`):**
  - Grading logic theo type — **all-or-nothing cho multi-slot, theo [QuestionContentSpec.md](./QuestionContentSpec.md):**
    ...
    - `FILL_NUMBER`: `blanks.every(b => strictNumericText(answer.blanks[b.id]) === b.correctValue)` — **exact Vietnamese decimal-comma format, exact precision, all-or-nothing**
